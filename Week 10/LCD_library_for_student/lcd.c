...
static void LCD_WR_REG(uint16_t LCD_Reg)
{
	...
	// LCD_CS, LCD_RS, LCD_WR을 Low로

	GPIO_Write(GPIOE, LCD_Reg); // Command 전송

	// TODO implement using GPIO_ResetBits/GPIO_SetBits
	GPIO_SetBits(GPIOD, GPIO_Pin_14);
	GPIO_SetBits(GPIOC, GPIO_Pin_8);
	// LCD_CS, LCD_WR을 다시 High로
}
...
static void LCD_WR_DATA(uint16_t LCD_Data)
{
	...
	GPIO_ResetBits(GPIOD, GPIO_Pin_14);
	// LCD_CS, LCD_WR을 Low로, LCD_RS를 High로
	
	GPIO_Write(GPIOE, LCD_Data); // Data 전송
